...
typedef GeometryTriBuffer::Index TriIndex;
...
public:
	...
	bool checkCompatibleNormals(GeometryTriBuffer& parent, const Vector3& refNormal, const float& planarAngle);
...
struct Edge {
	...
	bool updateCollapsable(const TriIndex& selfIndex, GeometryTriBuffer& parent, Edges& edges, EdgeVertices& edgeVertices, TriStates& triStates, 
	...
	bool tryCollapse(const TriIndex& selfIndex, GeometryTriBuffer& parent, Edges& edges, EdgeVertices& edgeVertices, TriStates& triStates, 
...
bool EdgeVertex::checkCompatibleNormals(GeometryTriBuffer& parent, const Vector3& refNormal, const float& planarAngle) {
	...
	for (TriIndex i = 0 ; i < vertices.size; ++i) {
		...
		const Vector3& normal = parent.get(vertices.el[i].tri).normal;
...
bool Edge::updateCollapsable(const TriIndex& selfIndex, GeometryTriBuffer& parent, Edges& edges, EdgeVertices& edgeVertices, TriStates& triStates, 
								const bool& useBatches, const TriIndex& batchIndex,
	...
	if (state == Normal) {
		...
		refNormal = parent.get(sharedTris[0]).normal;
		const Vector3& normal = parent.get(sharedTris[1]).normal;
...
bool Edge::tryCollapse(const TriIndex& selfIndex, GeometryTriBuffer& parent, Edges& edges, EdgeVertices& edgeVertices, TriStates& triStates, 
...
void simplify_Iterate(GeometryTriBuffer& parent, int limit, TriStates& triStates, EdgeVertices& edgeVertices, Edges& edges, 
...
void simplify_ClosestToCenter(GeometryTriBuffer& parent, int limit, TriStates& triStates, EdgeVertices& edgeVertices, Edges& edges, 
					  bool& centerIsValid, AAB& volume, Point& center, bool anythingGoes, bool useBatches, 
					  const float& planarAngle) {
...
void GeometryTriBuffer::simplify(const Simplification* _pSettings) {
