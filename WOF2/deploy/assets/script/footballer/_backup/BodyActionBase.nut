...
class BodyActionBase {
	...
	function switchTackled(footballer, t, tackler) {
		...
		stop(footballer, t);
		footballer.setAndStartAction(actionFalling, t, actionFalling.getStartStateFromFallTargetPos(footballer, tackler.impl.getPos()));
		
		return true;
	}
	
	function switchPainHitDir(footballer, t, hitDir) {
		...
		startState.fallDir.set(hitDir);
	
		stop(footballer, t);
		footballer.setAndStartAction(actionFalling, t, startState);
