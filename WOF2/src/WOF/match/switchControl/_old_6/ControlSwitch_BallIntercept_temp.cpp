	...
	if (mCurrSimulID != simul->getID()) {
		...
		while (iter.iterateFootballers(footballer.ptrRef(), index)) {
			...
			if (!consider) {
				...
				if (player.isValid() && player->hSwitchState.isSwitchLocked) {
		...
		for (FootballerIndex i = 0; i < mConsideredFootballers.count; ++i) {
			...
			if (player.isValid()) {
				...
				if (!player->hSwitchState.isSwitchLocked || match.mSwitchControlSetup.veryDynamic) {

					if (player->hSwitchState.switchLockFinished()) {

						//String::debug(L"switchunlocked %u\n", footballer->mNumber);
						doFullUpdate = true;
						break;
				...
				if (estimate.isPathValid()) {
					...
					if (!consider) {
						...
						if (player.isValid() && player->hSwitchState.isSwitchLocked) {
							...
							//String::debug(L"switchlocked -> stay %u\n", footballer->mNumber);
							consider = true;
						}
					}
