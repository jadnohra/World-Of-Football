...
class BodyAction_Tackling extends BodyActionBase {

	function getType(footballer) {
	
		return footballer.Action_Tackling;
	}
	...
	function start(footballer, t, startState, prevAction) {
		...
		mCurrPhysPhase.start(this, settings.PhysicsPhases, t);
		mCurrTacklePhase.start(this, settings.TacklePhases, t);
	...
	function checkColliders(footballer, t, isTacklePhase) {
		...
		while ((collider = impl.collFindDynVolColliderInt(index, footballerType)) != null) {
			...
			if (!(collider in mProcessedTackleColliders)) {
				...
				if (footballer.getPhysicalPowerTackle() + rand2f(footballer.tackle.TacklePowerRandomAmount) > collider.getPhysicalAgilityBalance()) {
	...
	function frameMove(footballer, t, dt) {
		...
		local isTacklePhase = (tacklePhase.id == settings.TacklePhase_Tackle);
		local isAnimFrozen = (physPhase.id == settings.PhysPhase_SlideMiddle);
		...
		{	
			...
			if (physPhase.id == settings.PhysPhase_SlideEnd) 
