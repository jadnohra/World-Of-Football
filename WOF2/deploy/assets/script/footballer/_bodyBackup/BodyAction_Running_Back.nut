...
class BodyAction_Running extends BodyActionBase_BallTriggers {
	...
	constructor() {
		...
		mRunDir = Vector3();
		mActiveBallShot = createFootballerBallShot();
	...
	function load(footballer, chunk, conv) {
		...
		mSettings = footballer.settings.run;
	
		return true;
	}
	
	function isFinished() { return true; }	
	function getStaminaGain(footballer) { return mSettings.getStaminaGain(footballer, mRunType, mRunSpeedType); }
	function isSwitchLocked(footballer) { return true; }
	
	function isStrafing(footballer) { return mSettings.isStrafing(mRunType); }
	function isStrafingHard(footballer) { return mSettings.isStrafingHard(mRunType); }
	...
	function setBodyFacing(footballer, val) {
		...
		request.validateZeroSceneUp(true);
	}
	...
	function setRunDir(footballer, val) {
		...
		if (val.isZero()) {
			...
			setRunDir(footballer, footballer.impl.getFacing());
			return;
		}
	...
	function setActiveBallShot(shot, shotVelScale, copyShot, queue) {
		...
		if (copyShot) {
			...
			mRequestActiveBallShot.value = createFootballerBallShot();
	...
	function setRunSpeed(footballer, val) {
		...
		mRequestRunSpeedType.value = val == null ? mSettings.RunSpeed_Normal : val;
	...
	function detectRunType(footballer, facing, dir) {
		...
		if (fabs(1.0 - dot) <= tolerance) {
			...
			type = mIsStrafing ? mSettings.Run_StrafeFwd : mSettings.Run_Normal;
			...
			local step = discretizeDirection(facing, footballer.impl.getMatch().SceneUpAxis(), dir, divCount, discreeteDir);
			step = step.tofloat();
		...
		if (type == mSettings.Run_Normal && footballer.impl.isBallOwner())
	...
	function updateRunState(footballer) {
		...
		if (runType != mRunType) {
			...
			if (animIndex != impl.getCurrAnimIndex()) {
				...
				impl.setAnimSpeed(mSettings.speedToAnimSpeed(footballer, mSettings.getSpeedValue( mRunType, mRunSpeedType)));
	...
	function applyStartState(footballer, startState) {
		...
		if (!runSpeedWasSet)
			setRunSpeed(footballer, mSettings.RunSpeed_Normal);
	}
	...
	function onHandledRequests(footballer, needUpdateRunState, needUpdateAnimSpeed) {
		...
		if (needUpdateAnimSpeed) {
			...
			footballer.impl.setAnimSpeed(mSettings.speedToAnimSpeed(footballer, mSettings.getSpeedValue(mRunType, mRunSpeedType)));
		}
	}
	
	function updateBodyMomentum(footballer, speedVector) {
	
		footballer.setBodyMomentum(speedVector, !mSettings.isStrafingHard(mRunType));
	}
	...
	function frameMove(footballer, t, dt) {
		...
		if (mRequestRunDir.isValid) {
			...
			if (dirDot != 0.0) {
				...
				print("dirDot: " + dirDot + ", eps: " + mSettings.RunDirChangeEpsilone);
			}
			*/
		...
		{	
			...
			/*			
			if (mSettings.EnableStrafeCorrectDribble && mStrafeDribbleCorrection != 0.0) {
				...
				if (runDirChanged) {
					...
					local correctionDist = dt * mSettings.StrafeCorrectDribbleSpeed;
	...
	********************/
	...
	function doBallActionStabilize(footballer, ball, soundName) {
		...
		footballer.shootBall(footballer.impl.genBallCommandID(), ball, shot, shot.speed, getBallActionDir(footballer), addedVel,
								true, true, false, footballer.getSkillBallProtection(), footballer.getSkillBallControlBodyOrFeet(), true, mSettings.DribbleShotSwitchInfluence, soundName);	
			
		//footballer.setBallCommandGhostedShot(footballer.impl.genBallCommandID(), ball, shot, shot.speed, mRunDir, addedVel, mSettings.DribbleShotSwitchInfluence, true, footballer.settings.BallSoftShotSoundName);
	}
	...
	function doBallActionShoot(footballer, ball, allowActiveShot) {
		...
		if (allowActiveShot && hasActiveBallShot()) {
			...
			//footballer.setBallCommandGhostedShot(footballer.impl.genBallCommandID(), ball, shot, shot.speed, mRunDir, addedVel, mSettings.DribbleShotSwitchInfluence, false, footballer.settings.BallShotSoundName);
		...
		switch (mBallControlState) {
			...
			case BallControlState_Owner: {
				...
				local shouldTriggerShot = impl.shouldTriggerBallDribbleAction(savedSpatialState.getWorldTransform(), impl.getWorldTransform(),
																		footballer.settings.BallSweetSpot, mRunDir, footballer.settings.BallPositionEpsilon, mSettings.RunDirChangeEpsilon);
