/*
	...
	Integrate this into Goalie Job code
	and change as planned
...
class FootballerBrain_AI_GoalGoalieJump extends FootballerBrain_AI_Goal {
	...
	constructor() {
		...
		mJumpStartState = BodyAction_Jumping_StartState();
	...
	function updateApproximateTarget(footballer, ball) {
	...
	function chooseJump(brain, footballer, t, dt) {
		...
		updateApproximateTarget(footballer, ball);
		...
		
		{
			...
			mJumpStartState.jumpHeight = mTunedEstimateTarget.getPos().getEl(::Scene_Up) - handPosAtApogee.getEl(::Scene_Up);
