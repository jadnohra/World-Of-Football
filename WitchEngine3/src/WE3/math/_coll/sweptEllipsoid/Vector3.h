...
class Vector3 : public WE::Vector3
{
public:

	// Constructor
	Vector3(bool dummyNoInit){}

	// Constructor
	
	Vector3(float inX = 0.0f, float inY = 0.0f, float inZ = 0.0f) 
			: WE::Vector3(inX, inY, inZ) 
	{ 
		/*
		mX = inX;
		mY = inY;
	...
	Vector3(const Vector2 &inOther) : WE::Vector3(inOther.mX, inOther.mY, 0.0f) 
		//mX(inOther.mX),
		//mY(inOther.mY),
		//mZ(0.0f)
	{
	...
	inline void operator = (const WE::Vector3 &inOther) 
	{
		mX = inOther.x;
		mY = inOther.y;
		mZ = inOther.z;
	...
	inline void copyTo (WE::Vector3 &out) 
	{
		out.x = mX;
		out.y = mY;
		out.z = mZ;
	...
	inline void subtract(const WE::Vector3& b, WE::Vector3& selfMinusB) {

		selfMinusB.x = mX - b.mX;
		selfMinusB.y = mY - b.mY;
		selfMinusB.z = mZ - b.mZ;
	}
	...
	// Dot product
	inline float			Dot(const WE::Vector3 &inOther) const
	{
		return mX * inOther.mX + mY * inOther.mY + mZ * inOther.mZ;
	}
