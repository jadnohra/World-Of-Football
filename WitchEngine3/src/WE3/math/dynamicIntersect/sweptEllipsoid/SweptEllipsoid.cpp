...
namespace WE_3rdp_SweptEllipsoid {
...
bool PlaneSweptSphereIntersect(const Plane &inPlane, const Vector3 &inBegin, const Vector3 &inDelta, float inRadius, float &outT1, float &outT2)
{
	...
	const float n_dot_d_epsilon = sinf(WE::degToRad(0.005f));
	const float radius_epsilon = inRadius * 0.01f;

	if (n_dot_d == 0.0f)
	//if (fabs(n_dot_d) < n_dot_d_epsilon)
	{
		...
		//	return false;
		if (fabs(dist_to_b) > inRadius + radius_epsilon)
			return false;
	...
	{
		//added to make circle radius never be exactly zero
		//if (dist_to_b == inRadius) {
	
		//	dist_to_b = inRadius - 0.005f * inRadius;
		//}
...
bool SweptCircleEdgeVertexIntersect(const Vector2 *inVertices, int inNumVertices, const Vector2 &inBegin, const Vector2 &inDelta, float inA, float inB, float inC, Vector2 &outPoint, float &outFraction, unsigned short& e1, unsigned short& e2, bool& isPoint)
{

	short _e1, _e2;

	_e1 = 0;
	_e2 = (short) (inNumVertices - 1);

	// Loop through edges
	float upper_bound = 1.0f;
	bool collision = false;
	for (const Vector2 *v1 = inVertices, *v2 = inVertices + inNumVertices - 1; 
					v1 < inVertices + inNumVertices; 
					v2 = v1, _e2 = _e1, ++v1, ++_e1)
	{
		float t;
...
bool PolygonSweptSphereIntersect(const Plane &inPlane, const Vector2 *inVertices, int inNumVertices, const Vector3 &inBegin, const Vector3 &inDelta, float inRadius, Vector3 &outPoint, float &outFraction, PlanarFaceIntersection& info)
{
	...
	//	dist_to_b = inRadius - 0.01f * inRadius;
	//}
	...
	Vector2 p;
	/*
	float inRadiusSq = a * t1 * t1 + b * t1 + c;
	if (inRadiusSq == 0.0f) {
		inRadiusSq = 0.01f * inRadius * inRadius;
	}
	*/
	if (PolygonCircleIntersect(inVertices, inNumVertices, begin + delta * t1, a * t1 * t1 + b * t1 + c, p))
	{
		...
		info.type= PlanarFaceIntersection::T_Face;
	...
	bool isPoint;
	if (SweptCircleEdgeVertexIntersect(inVertices, inNumVertices, begin, delta, a, b, c, p, outFraction, info.edge[0], info.edge[1], isPoint))
	{
		outPoint = inPlane.ConvertPlaneToWorld(u, v, p);

		//type.isVertexNormal = false;
		if (isPoint) {
			
			//type.edge[1] = (type.edge[0] + 1) % inNumVertices;
			info.type= PlanarFaceIntersection::T_Point;
		} else {

			info.type= PlanarFaceIntersection::T_Edge;
		}
...
bool PolygonSweptEllipsoidIntersect(const Plane &inPlane, const Vector2 *inVertices, int inNumVertices, const Vector3 &inBegin, const Vector3 &inDelta, const Vector3 &inAxis1, const Vector3 &inAxis2, const Vector3 &inAxis3, Vector3 &outPoint, float &outFraction)
{
	...
	// Test if sphere intersects with one of the edges or vertices
	if (SweptCircleEdgeVertexIntersect(transformed_vertices, inNumVertices, begin, delta, a, b, c, p, outFraction, temp[0], temp[1], tempIsPoint))
	{
		outPoint = unit_sphere_to_world * (transformed_plane_to_unit_sphere * Vector3(p + trans));
		return true;
	}
