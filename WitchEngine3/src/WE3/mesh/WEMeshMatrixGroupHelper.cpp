...
struct skel_GroupContext {

	bool isIndexedBlending;
	int maxGroupSize;
	int blendMatrixCount;
};
...
void skel_processFace_removeBone(skel_Context& ct, float* weights, BYTE* indices, SceneTransformIndex index, float weight) {
	...
	while (addedWeight != 0.0f) {
		...
		if ((index < ct.expWeightCount) && !lastWeightIsZero &&lastWeight > max) {
...
bool skel_groupN_processFace_adapt(skel_Context& ct, skel_GroupContext& groupCt, skel_FaceBoneGroup& group, float* weights, BYTE* indices) {

	if (groupCt.isIndexedBlending) {
		...
		for (i = 0; i < ct.expWeightCount; i++) {
			...
			if (weights[i] > 0.0f) {
				...
				for (j = 0; j < group.bones.count; j++) {
					...
					if (group.bones.el[j].bone == indices[i]) {
		...
		WETL::SizeAllocT<float, int> origWeights;
		WETL::SizeAllocT<BYTE, int> origIndices;

		origWeights.resize(ct.expWeightCount + 1);
		origIndices.resize(ct.expWeightCount + 1);

		memcpy(origWeights.el, weights, ct.expWeightCount * sizeof(float));
		origWeights.el[ct.expWeightCount] = lastWeight;

		memcpy(origIndices.el, indices, (ct.expWeightCount + 1) * sizeof(BYTE));

		for (j = 0; j < group.bones.count && j < ct.expWeightCount + 1; j++) {
			indices[j] = group.bones.el[j].bone;
...
bool skel_groupN_processFace_tryAdapt(skel_Context& ct, skel_FaceBoneGroup& group, 
											   skel_FaceBoneGroup& groupToAdapt) {

	SceneTransformIndex i;
	SceneTransformIndex j;
	bool boneFound;

	int unfoundBoneCount = 0;
	...
	if (unfoundBoneCount == 0)
		return true;

	if (group.bones.count + unfoundBoneCount > group.maxCount)
		return false;
	...
	for (i = 0; i < groupToAdapt.bones.count; i++) {
		...
		if (boneFound == false) {
			...
			skel_FaceBone& newBone = group.bones.addOne();
			newBone = groupToAdapt.bones.el[i];
		} 
	}
...
WETL::Index::Type skel_groupN_processFace_findAdaptable(skel_Context& ct, skel_GroupContext& groupCt, skel_FaceBoneGroupArray& groupArray, 
										 skel_FaceBoneGroup& groupToAdapt) {

	bool adapted = false;
	WETL::Index g;
	for (g = 0; g < groupArray.count; g++) {
		if (skel_groupN_processFace_tryAdapt(ct, *groupArray.el[g], groupToAdapt)) {
			return g;
		}
	}

	g = groupArray.count;
	groupArray.addOne(new skel_FaceBoneGroup(groupCt.maxGroupSize));
	if (skel_groupN_processFace_tryAdapt(ct, *groupArray.el[g], groupToAdapt) == false) {
		//ct.processingCosts.unadaptableFaceCount++;
	}
...
void skel_groupN_mergeFace(skel_Context& ct, skel_GroupContext& groupCt, UINT32 faceIndices[3], skel_FaceBoneGroup& mergedNonZeroWeightGroup, int* pLimitGroupBoneCount) {
	...
	if (pLimitGroupBoneCount) {

		if (mergedNonZeroWeightGroup.bones.count > *pLimitGroupBoneCount) {

			mergedNonZeroWeightGroup.sortSmart(false);
			mergedNonZeroWeightGroup.bones.count = *pLimitGroupBoneCount;
		}
	}
}
...
struct skel_FaceProcessInfoN : skel_FaceGroupLink {

	bool processed;
	bool merged;
	skel_FaceBoneGroup mergedGroup;


	skel_FaceProcessInfoN(int maxGroupSize = 0) : mergedGroup(maxGroupSize) {
	...
	void init(int maxGroupSize) {

		processed = false;
		merged = false;
		
		mergedGroup.bones.count = 0;
		mergedGroup.init(maxGroupSize);
	}
};
...
bool skel_groupN_processFace(skel_Context& ct, skel_GroupContext& groupCt, RenderUINT faceIndex, UINT32 faceIndices[3], skel_FaceBoneGroupArray& groupArray, 
								   skel_FaceProcessInfoN& processInfo, skel_VertexProcessInfo* vertexProcessInfos, 
								   skel_AddedVertex4Array& addedVertices, int minBoneCount = -1) {
	...
	if (processInfo.merged == false) {
		skel_groupN_mergeFace(ct, groupCt, faceIndices, processInfo.mergedGroup, groupCt.isIndexedBlending ? NULL : &groupCt.maxGroupSize);
	...
	processInfo.groupIndex = (int) skel_groupN_processFace_findAdaptable(ct, groupCt, groupArray, processInfo.mergedGroup);
	...
	for (int v = 0; v < 3; v++) {
		...
		if (vertexProcessInfos[faceIndices[v]].adaptedGroup == -1) {
			...
			if (!groupCt.isIndexedBlending) {
				...
				skel_groupN_processFace_adapt(ct, groupCt, *groupArray.el[processInfo.groupIndex], weights, indices);
			}
			...
			if (vertexProcessInfos[faceIndices[v]].adaptedGroup != processInfo.groupIndex) {
				...
				if (!groupCt.isIndexedBlending) {
					...
					skel_groupN_processFace_adapt(ct, groupCt, *groupArray.el[processInfo.groupIndex], pAdded->weights, pAdded->indices);
				}
			}
		}
	}
...
void skel_groupN_postProcessFace(skel_Context& ct, skel_GroupContext& groupCt, RenderUINT faceIndex, UINT32 faceIndices[3], skel_FaceBoneGroupArray& groupArray, 
								   skel_FaceProcessInfoN& processInfo, skel_VertexProcessInfo* vertexProcessInfos, RenderUINT origVertexCount, 
								   skel_AddedVertex4Array& addedVertices, int minBoneCount = -1) {

	if (groupCt.isIndexedBlending) {

		assrt(processInfo.processed);
		...
		for (int v = 0; v < 3; v++) {
			...
			RenderUINT vertexIndex = faceIndices[v];

			bool needsRemapping = false;

			if (vertexIndex < origVertexCount) {

				skel_VertexProcessInfo& vertexInfo = vertexProcessInfos[vertexIndex];

				if (vertexInfo.postProcessed) {

					needsRemapping = false;

				} else {

					vertexInfo.postProcessed = true;
					needsRemapping = true;
				}

			} else {

				//is an added vertex only member of this face
				needsRemapping = true;
			}

			if (needsRemapping) {

				weights = ct.vprofile->getF(ct.vb, vertexIndex, ct.el_weights);
				indices = ct.vprofile->getB(ct.vb, vertexIndex, ct.el_indices);
...
void skel_groupN_mergeAddedVerticesAndRemoveBlendIndices(skel_Context& ct, skel_GroupContext& groupCt, skel_AddedVertex4Array& addedVertices) {
	...
	if (!groupCt.isIndexedBlending && ct.el_indices != RenderElSubNull) {
	...
	short newel_weights = newProfile.findSub(D3DDECLUSAGE_BLENDWEIGHT, 0);
	short newel_indices = newProfile.findSub(D3DDECLUSAGE_BLENDINDICES, 0);
	int newWeightOffset = newProfile.mSubElements.el[newel_weights].Offset;
	int newIndicesOffset = 0;
	bool newHasIndices = newel_indices != RenderElSubNull;
	if (newHasIndices)
		newIndicesOffset = newProfile.mSubElements.el[newel_indices].Offset;
	...
	if (profileChanged) { 
		...
		pDest += ct.vertexCount * newProfile.mElementSizeBytes;
		...
		for (WETL::Index a = 0; a < addedVertices.count; a++) {
			...
			if (newHasIndices)
				memcpy(pDest + newIndicesOffset, addedVertices.el[a].indices, (ct.expWeightCount + 1)* sizeof(BYTE));
...
void skel_groupN(skel_Context2& ct, skel_GroupContext& groupContext, StaticMeshMatrixGroupArray& blendParts) {

	skel_FaceBoneGroupArray groupArray;
	UINT32 faceIndices[3];
	...
	if (ct.faceCount > 0) {
		...
		RenderUINT processedFaceCount = 0;
		skel_FaceProcessInfoN* facePorcessInfos;
		MMemNew(facePorcessInfos, skel_FaceProcessInfoN[faceCount]);
		skel_VertexProcessInfo* vertexProcessInfos;
		MMemNew(vertexProcessInfos, skel_VertexProcessInfo[ct.vertexCount]);

		for (RenderUINT face = 0; face < ct.faceCount; face++) {

			facePorcessInfos[face].init(groupContext.maxGroupSize);
		}
		...
		*/
		...
		//for (int bc = ct.expWeightCount + 1; bc >= 0; bc--) {
			...
			for (RenderUINT face = 0, i = 0; face < ct.faceCount; face++, i += 3) {
				...
				if (skel_groupN_processFace(ct, groupContext, face, faceIndices, groupArray, facePorcessInfos[face], 
						vertexProcessInfos, addedVertices, -1)) {
		...
		RenderUINT origVertexCount = ct.vertexCount;

		skel_groupN_mergeAddedVerticesAndRemoveBlendIndices(ct, groupContext, addedVertices);

		if (groupContext.isIndexedBlending) {

			//remap absolute bone indices to group indices
			for (RenderUINT face = 0, i = 0; face < ct.faceCount; face++, i += 3) {

				ct.iprofile->face_get(ct.ib, i, faceIndices);
								
				skel_groupN_postProcessFace(ct, groupContext, face, faceIndices, groupArray, facePorcessInfos[face], 
						vertexProcessInfos, origVertexCount, addedVertices, -1);
			}
		}
		...
		for (SceneTransformIndex g = 0; g < groupArray.count; g++) {
			...
			pBlendState->setIndexedBlending(groupContext.isIndexedBlending, groupContext.blendMatrixCount);
...
void skel_limitFaceWeights_old(skel_Context2& ct, int max, bool limitVertexWeights) {
...
void skel_extractVertexBlendQueryInfo(MeshGeometryVertexBuffer& VB, RenderVertexBlendingQuery& blendQuery) {
	...
	blendQuery.inUseIndexed = (el_indices != RenderElSubNull);
...
bool MeshMatrixGroupHelper::prepareMatrixGroupsForRenderer(
	...
	if (!blendQuery.outIsSupported || !blendQuery.outVertexBlend) {
	...
	*/
	...
	if (blendQuery.outUseIndexed) {
		...
		if (blendQuery.outIndexedSplit) {
			...
			if (blendQuery.inMaxBlendMatrixCount != blendQuery.outMaxBlendMatrixCount)
				skel_limitFaceWeights(ct, blendQuery.outIndexedSplitMaxMatrixCount, true);

			skel_GroupContext groupCt;

			groupCt.isIndexedBlending = true;
			groupCt.maxGroupSize = blendQuery.outIndexedSplitMaxMatrixCount;
			groupCt.blendMatrixCount = ct.expWeightCount + 1;

			skel_groupN(ct, groupCt, resultMatrixGroups);
			...
			if (blendQuery.inMaxBlendMatrixCount != blendQuery.outMaxBlendMatrixCount) {
				...
				skel_limitFaceWeights(ct, blendQuery.inTotalMatrixCount, true);
			}
			...
			pGroup->setIsIdentityMatrixMapping(true);
		...
		skel_limitFaceWeights(ct, blendQuery.outMaxBlendMatrixCount, true || blendQuery.inMaxBlendMatrixCount != blendQuery.outMaxBlendMatrixCount);
		//skel_group4(ct, resultMatrixGroups);

		skel_GroupContext groupCt;

		groupCt.isIndexedBlending = false;
		groupCt.blendMatrixCount = ct.expWeightCount + 1;
		groupCt.maxGroupSize = groupCt.blendMatrixCount;
...
bool MeshMatrixGroupHelper::getRenderReadyBuffers(Renderer& renderer, CoordSysConvPool& convPool,
	...
	if (!MeshMatrixGroupHelper::prepareMatrixGroupsForRenderer(renderer, renderReadyVB.dref(), renderReadyIB.dref(), FB, 
