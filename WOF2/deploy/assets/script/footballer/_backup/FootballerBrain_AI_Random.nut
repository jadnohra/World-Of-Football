				...
				switch (newState) {
					...
					default: {
						...
						
						{
							...
							if (fabs(pos.get(match.Scene_Right)) > match.getPitchHalfWidth()) {
						
								if (pos.get(match.Scene_Right) > 0.0) {
								
									if (startState.runDir.get(match.Scene_Right) > 0.0) {
									
										startState.runDir.setEl(match.Scene_Right, -startState.runDir.get(match.Scene_Right));
									}
							...
							} else if (fabs(pos.get(match.Scene_Fwd)) > match.getPitchHalfLength()) {
							
								if (pos.get(match.Scene_Fwd) > 0.0) {
								
									if (startState.runDir.get(match.Scene_Fwd) > 0.0) {
									
										startState.runDir.setEl(match.Scene_Fwd, -startState.runDir.get(match.Scene_Fwd));
									}
