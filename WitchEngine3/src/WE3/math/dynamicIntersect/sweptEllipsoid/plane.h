...
public:
	...
	// Convert a point from world space to plane space (3D -> 2D)
	inline static void sConvertWorldToPlane(const Vector3 &inU, const Vector3 &inV, const WE::Vector3 &inPoint, Vector2& planePt)
	{
		planePt.mX = inU.Dot(inPoint);
		planePt.mY = inV.Dot(inPoint);
	}
	...
	// Convert a point from world space to plane space (3D -> 2D)
	inline void ConvertWorldToPlane(const WE::Vector3 &inPoint, Vector2& planePt) const
	{
		Vector3 u, v;
		GetBasisVectors(u, v);
		sConvertWorldToPlane(u, v, inPoint, planePt);
	}
