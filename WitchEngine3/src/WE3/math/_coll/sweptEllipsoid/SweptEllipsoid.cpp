...
bool PolygonSweptSphereIntersect(const Plane &inPlane, const Vector2 *inVertices, int inNumVertices, const Vector3 &inBegin, const Vector3 &inDelta, float inRadius, Vector3 &outPoint, float &outFraction, ContactInfo& info)
{
	...
	*/
	...
	if (SweptCircleEdgeVertexIntersect(inVertices, inNumVertices, begin, delta, a, b, c, p, outFraction, info.edge[0], info.edge[1], isPoint))
	{
		...
		if (isPoint) {
			...
			info.type= ContactInfo::T_Point;
		} else {

			info.type= ContactInfo::T_Edge;
		}
