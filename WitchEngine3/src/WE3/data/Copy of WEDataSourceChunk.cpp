...
StringHash DataSourceChunk::getSourcePathId()  {

	return (mSource->getSourcePathId());
}
...
StringHash DataSourceChunk::extractId(const TCHAR* attribute)  {
	...
	String idStr;
	if (mChunk->getAttribute(attribute, idStr)) {
	...
	return getSourcePathId();
}

bool DataSourceChunk::extractId(String& id, const TCHAR* attribute)  {
	...
	return mChunk->getAttribute(attribute, id);
}


bool DataSourceChunk::resolveFilePath(String& filePath) {
...
bool DataSourceChunk::resloveRemoteChunk(DataSourceChunk& target, const TCHAR* fileAttr, const TCHAR* chunkAttr) {
	...
	if (mChunk->hasChildren()) {
		...
		if (hasAttrFile) {
			...
			if (resolveFilePath(attrFile) == false) {
