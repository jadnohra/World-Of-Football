...
class FootballerBallShot {
	...
	function getShotConfiguration(ball, shotV, shotVMag, maxShotSpeed) {
	...
	function syncShotWToShotV(ball, shotV, shotW, shotConfig, residualFactor) {
	
		if (wHorizRatio && wVertRatio) {
			
			local temp = Vector3();
					
			ball.calcVelocitySyncedW(shotV, ::Scene_UpAxis, temp);
			temp.mulToSelf(wHorizRatio);
			
			ball.calcVelocitySyncedW(shotV, ::Scene_RightAxis, shotW);
			...
			ball.calcVelocitySyncedW(shotV, ::Scene_UpAxis, shotW);
			shotW.mulToSelf(wVertRatio);
	...
	function getVelocity(maxShotSpeed, ball, shotDir, _shotSpeed, addedVel, shotV, shotW, speedVariance, dirVariance, controlSkill, allowDownShot) {
	
		local shotSoundMag = 0.0;
		local isHeader = type == ::FootballerBallShot_Header;
		local ballSpeed = ball.getVel().mag();
		local maxShotSpeed = maxf(ballSpeed, maxShotSpeed);
		...
		::Scene_UpAxis.mulAndAdd(shotSpeed * (1.0 - horizRatio), shotV);
		...
		local shotConfig = getShotConfiguration(ball, shotV, isHeader ? _shotSpeed : shotSpeed, maxShotSpeed);
		...
		if (dirVariance != 0.0) {
			...
			if (!allowDownShot) {
				...
				if (shotV.getEl(::Scene_Up) < 0.0) {
				
					shotV.setEl(::Scene_Up, - shotV.getEl(::Scene_Up));
				}
			}
		}
		...
		if (controlSkill < 1.0) {
			...
			if (residualV.dot(shotV) < 0.0) {
				...
				if (invRight)
					residualV.setEl(::Scene_Right, -residualV.getEl(::Scene_Right));
				
				if (residualV.dot(shotV) < 0.0) 
					residualV.setEl(::Scene_Fwd, -residualV.getEl(::Scene_Fwd));
		...
		syncShotWToShotV(ball, shotV, shotW, shotConfig, residualFactor);
...
function createFootballerBallShot() {

	return FootballerBallShot(false, ::FootballerBallShot_None, 0.0, 0.0, 0.0, 0.0, false);
}

class FootballerBallShotRequestValue {
