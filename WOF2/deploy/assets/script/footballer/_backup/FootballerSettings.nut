...
class FootballerSettings {
	...
	Shots = [
		...
		[
			...
			//Shot_Extra
			FootballerBallShot(true, ::FootballerBallShot_Foot, 27.5, 30.0, 0.0, -0.1, true)
		],
	...
	function load(match, conv) {
	
		BallSweetSpot.setEl(match.Scene_Right, 0.10);
		BallSweetSpot.setEl(match.Scene_Up, 0.0); //will be set properly on prepareScene
		BallSweetSpot.setEl(match.Scene_Fwd, 0.45);
		conv.convVector3Units(BallSweetSpot);
		
		BallSweetSpottingSpeed = conv.convUnit(BallSweetSpottingSpeed);
		
		COCOffset.zero();
		COCOffset.setEl(match.Scene_Fwd, 0.15);
		conv.convVector3Units(COCOffset);
		
		BallHeaderHeight = conv.convUnit(BallHeaderHeight);
		
		BallFootControlHeight = conv.convUnit(BallFootControlHeight);
		BallFootControlDistMin = conv.convUnit(BallFootControlDistMin);
		BallFootControlDistMax = conv.convUnit(BallFootControlDistMax);
		
		BallPainSpeedSq = conv.convUnit(BallPainSpeedSq);
		BallPainSpeedSq *= BallPainSpeedSq;
		
		BallPositionEpsilon = conv.convUnit(BallPositionEpsilon);
		...
		BallAfterTouchSideMag = conv.convUnit(BallAfterTouchSideMag);
		BallAfterTouchUpMag = conv.convUnit(BallAfterTouchUpMag);
		BallAfterTouchDownMag = conv.convUnit(BallAfterTouchDownMag);
	}
	
	function prepareScene(match) {
	
		BallSweetSpot.setEl(match.Scene_Up, match.getBall().getRadius());
	}
	
	function isSwitchLockingActionType(footballer, actionType) {
		
		return actionType != footballer.Action_Idle;
	}
	...
	function addTriggerVolumesFeet(footballer, leftTriggerID, rightTriggerID) {
	...
	
		{
			...
			position.zero();
			position.setEl(match.Scene_Up, match.conv(0.2));
		
			impl.addTriggerVolume(leftTriggerID, "l_foot", position, radius);
			
			radius = match.conv(0.42);
			position.setEl(match.Scene_Up, match.conv(-0.2));
			position.setEl(match.Scene_Fwd, match.conv(-0.12));
			impl.addTriggerVolume(rightTriggerID, "r_foot", position, radius);
		}
	}
	
	function addTriggerVolumeHead(footballer, triggerID) {
	...
	
		{
			...
			position.zero();
			position.setEl(match.Scene_Up, match.conv(0.2));
			position.setEl(match.Scene_Right, match.conv(-0.25));
					
			impl.addTriggerVolume(triggerID, "head", position, radius);
		}
	}
	
	function addTriggerVolumes(footballer) {
	...
	
		{
			...
			position.setEl(match.Scene_Up, match.conv(0.3));
			position.setEl(match.Scene_Right, match.conv(-0.2));
		...
		*/
		...
		{
			radius = match.conv(0.25);
			position.setEl(match.Scene_Fwd, match.conv(0.3));
			position.setEl(match.Scene_Up, match.conv(0.25));
		
			position.setEl(match.Scene_Right, match.conv(-0.3));
		...
		
		{
			...
			position.setEl(match.Scene_Fwd, match.conv(0.3));
			position.setEl(match.Scene_Up, match.conv(0.7));
		...
		
		{
			...
			position.setEl(match.Scene_Fwd, match.conv(0.2));
			position.setEl(match.Scene_Up, match.conv(1.1));
		
			position.setEl(match.Scene_Right, match.conv(-0.1));
		...
		
		{
			...
			position.setEl(match.Scene_Fwd, match.conv(0.1));
			position.setEl(match.Scene_Up, match.conv(1.45));
			position.setEl(match.Scene_Right, match.conv(0.0));
		...
		
		{
			radius = match.conv(0.05);
			position.setEl(match.Scene_Fwd, match.conv(0.1));
			position.setEl(match.Scene_Up, match.conv(0.85));
			position.setEl(match.Scene_Right, match.conv(0.0));
